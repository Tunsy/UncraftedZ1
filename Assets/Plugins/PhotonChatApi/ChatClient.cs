...
namespace Assets.Plugins.PhotonChatApi
{
    ...
    public class ChatClient : IPhotonPeerListener
    {
        ...
        public readonly Dictionary<string, ExitGames.Client.Photon.Chat.ChatChannel> PublicChannels;
        public readonly Dictionary<string, ExitGames.Client.Photon.Chat.ChatChannel> PrivateChannels;

        private readonly ExitGames.Client.Photon.Chat.IChatClientListener listener = null;
        ...
        public ChatClient(ExitGames.Client.Photon.Chat.IChatClientListener listener, ConnectionProtocol protocol =
#if UNITY_WEBGL
    ...
    ConnectionProtocol.Udp
        ...
        )
        {
            ...
            this.PublicChannels = new Dictionary<string, ExitGames.Client.Photon.Chat.ChatChannel>();
            this.PrivateChannels = new Dictionary<string, ExitGames.Client.Photon.Chat.ChatChannel>();
        }
        ...
        private void HandlePrivateMessageEvent(EventData eventData)
        {
            ...
            ExitGames.Client.Photon.Chat.ChatChannel channel;
            if (!this.PrivateChannels.TryGetValue(channelName, out channel))
            {
                channel = new ExitGames.Client.Photon.Chat.ChatChannel(channelName);
        ...
        private void HandleSubscribeEvent(EventData eventData)
        {
            ...
            for (int i = 0; i < channelsInResponse.Length; i++)
            {
                if (results[i])
                {
                    ...
                    {
                        ExitGames.Client.Photon.Chat.ChatChannel channel = new ExitGames.Client.Photon.Chat.ChatChannel(channelName);
                        channel.MessageLimit = this.MessageLimit;
